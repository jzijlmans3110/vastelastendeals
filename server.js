...
// ── HEALTH ────────────────────────────────────────────────────────────────────
app.get('/', (req, res) => {
  res.json({ status: 'AI Beller + Calendar actief', versie: '2.1.0' });
});
...
// ── CLAUDE PROXY VOOR FRONTEND / LOVABLE ──────────────────────────────────────
...
// ── ELEVENLABS TTS PROXY VOOR FRONTEND / LOVABLE ──────────────────────────────
...
// ── HELPER: GESPREK MAKEN EN TWILIO CALL STARTEN ──────────────────────────────
...
// ── GESPREK STARTEN (NIEUWE LOVABLE ROUTE) ────────────────────────────────────
app.post('/api/call', async (req, res) => {
  try {
    ...
    console.error('API call fout:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/twilio/answer/:gesprekId', async (req, res) => {
  ...
  try {
    ...
    twiml.say(
      ...
      'Goedemiddag, Eva van ' +
        (process.env.BEDRIJF_NAAM || 'Vaste Lasten Deals') +
        '. Excuses, wij bellen later terug.'
    );
    twiml.hangup();
    res.type('text/xml').send(twiml.toString());
  }
...
app.post('/twilio/gather/:gesprekId', async (req, res) => {
  ...
  if (optOut.some(w => klantTekst.toLowerCase().includes(w))) {
    const tekst =
      'Ik begrijp het volledig. Uw nummer wordt direct verwijderd. Excuses voor het ongemak en een fijne dag!';
  ...
  if (isAkkoord && gesprek.wachtOpSlotKeuze && gesprek.vrijeSlots?.length) {
    ...
    if (gekozenSlot) {
      try {
        ...
        const bevestiging =
          'Uitstekend! Ik heb de afspraak voor u ingepland op ' +
          gekozenSlot.label +
  ...
  const wiltAfspraak =
    afspraakWoorden.some(w => klantTekst.toLowerCase().includes(w)) ||
  ...
  try {
    ...
    if (maxBereikt) {
      gesprek.status = 'afgerond';
      gesprek.resultaat = 'geen_interesse';
...
app.post('/api/bulk-start', async (req, res) => {
  ...
  let vrijeSlots = [];
  try {
    vrijeSlots = await getVrijeSlots(7, 60);
  } catch (e) {
  ...
  for (let i = 0; i < leads.length; i++) {
    setTimeout(async () => {
      try {
        let tel = String(leads[i].telefoon || '').replace(/[\s\-]/g, '');
        if (tel.startsWith('0')) tel = '+31' + tel.slice(1);
        if (!tel.startsWith('+')) tel = '+' + tel;
      ...
      ? [
          {
            role: 'user',
            content:
              'Begin het gesprek. Stel jezelf voor als Eva van ' +
              (process.env.BEDRIJF_NAAM || 'Vaste Lasten Deals') +
              ' en leg het doel uit.'
          }
        ]
...
async function spreekEnLuister(twiml, tekst, gesprekId) {
  if (process.env.ELEVENLABS_API_KEY && process.env.ELEVENLABS_VOICE_ID) {
    try {
      twiml.play(await genAudio(tekst, gesprekId));
    } catch {
...
async function spreekEnSluitAf(twiml, tekst) {
  if (process.env.ELEVENLABS_API_KEY && process.env.ELEVENLABS_VOICE_ID) {
    try {
      twiml.play(await genAudio(tekst, 'af_' + Date.now()));
    } catch {
      twiml.say({ language: 'nl-NL', voice: 'Polly.Lotte' }, tekst);
    }
  } else {
